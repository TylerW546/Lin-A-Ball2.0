...
class Enemy:
    ...
    def __init__(self, r=25, c=Global.GREEN, s=5):
        ...
        self.radius = r
        self.color = c
        self.x = random.randint(r, Global.SCREEN_WIDTH - r)
        self.y = random.randint(r, Global.SCREEN_HEIGHT - r)
        self.speed = s
    ...
    def bounce(self):
        ...
        if self.x < self.radius or self.x > Global.SCREEN_WIDTH - self.radius:
            self.dx = -self.dx
            if random.randint(1,2) == 1:
                self.dy = max( -18, min( self.dy + random.randint(-6,6),18) )
            self.x = max(self.radius, min(Global.SCREEN_WIDTH - self.radius, self.x))
        
        if self.y < self.radius or self.y > Global.SCREEN_WIDTH - self.radius:
            self.dy = -self.dy
            if random.randint(1,2) == 1:
                self.dx = max( -18, min( self.dx + random.randint(-6,6),18) )
                
            self.y = max(self.radius, min(Global.SCREEN_HEIGHT - self.radius, self.y))
